...
def get_cultural_facilities_in_area(area_name, page):
    ...
    # API 호출을 위한 파라미터 설정
    params = {
        'query': area_name + '문화시설',  # 검색할 키워드 (특정 구역명 + 문화시설)
        'category_group_code': 'CT1',  # 문화시설 카테고리 그룹 코드 (CT1: 문화시설)
...
def append_unique_facilities_to_csv(facilities, file_path):
    ...
    # 새로운 문화시설 정보 중에서 기존에 없는 것만 추가하기
...
def remove_duplicate_entries(file_path):
    # 중복을 제거할 문화시설 정보를 담을 집합
    unique_facilities = set()

    # CSV 파일에서 문화시설 정보 읽어오기
    with open(file_path, 'r', newline='', encoding='utf-8') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            unique_facilities.add((row['name'], row['address']))

    # 중복을 제거한 문화시설 정보를 다시 CSV 파일에 저장하기
