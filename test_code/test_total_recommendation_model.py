...
# 모델 저장 경로
model_dir = 'model'
model_path = os.path.join(model_dir, 'total_recommendation_model.pth')
...
def recommend_accommodations(model, data, station_name, accommodation_type, top_n=3):
    ...
    filtered_data['predicted_rating'] = predictions
    top_accommodations = filtered_data.sort_values(by='predicted_rating', ascending=False).head(top_n)
